pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'web-back-orders-k8s'
        GITHUB_REPO = 'https://github.com/jbnu-web-class-project/orders.git'
                ...
                script {
                    sshagent(credentials: [SSH_CREDENTIALS_ID]) {
                        ...
                        script {
                            ...
                            cd k8s-manifest &&
                            ls -l &&
                            sed -i 's|image: .*\$|image: ${HARBOR_REPO}/heim/${IMAGE_NAME}:${env.BUILD_ID}|' orders/back-orders-deploy.yaml &&
                            git config --global user.email "gjdhks1212@gmail.com"
                            git config --global user.name "hodu26"
                            git add orders/back-orders-deploy.yaml &&
